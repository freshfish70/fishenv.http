...
- **Pre-compressed files**: Serve `.gz` / `.br` variants automatically when
  `Accept-Encoding` matches.
- **Security headers**: Add `X-Content-Type-Options: nosniff` and
  `Cache-Control: no-cache` by default; user can override.
...
async function serveStaticFile(
  ...
  const { resolvedPath, encoding } = await resolveCompressed(
    filePath,
    acceptEncoding,
  );

  const mime = contentType(path.extname(filePath)) ??
...
async function serveDirectoryListing(
  ...
  const items = entries
    .map((e) =>
      `<li><a href="${e.name}${e.isDirectory ? "/" : ""}">${e.name}</a></li>`
    )
    .join("\n");
  return new Response(
...
- `static/mod.ts` — `withStatic()`, `serveStaticFile()`, `resolveSafePath()`,
  `resolveCompressed()`, `serveDirectoryListing()`
- `static/static.test.ts` — path traversal blocked, dot-file blocking,
  pre-compressed serving, ETag 304, directory listing HTML/JSON
