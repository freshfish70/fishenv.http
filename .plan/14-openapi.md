...
- **Schema adapter**: Bundle a Valibot-to-JSON-Schema adapter. We're
  opinionated; users with other libs provide a custom adapter.
- **UI**: Serve JSON spec only. No Swagger/Scalar HTML.
- **OperationId**: Auto-generated from method + path (e.g. `post_api_users_id`);
  overridable via `.meta({ operationId })`.
- **Error types in `.output()`**: Documentation only — used to generate
  `responses` error entries in the spec.
...
function buildOperation(
  ...
  return {
    operationId: route.meta?.operationId ??
      generateOperationId(route.method, route.fullPath),
...
function buildParameters(
  ...
  if (route.inputOptions?.query) {
    ...
    for (
      const [name, propSchema] of Object.entries(jsonSchema.properties ?? {})
    ) {
      params.push({
        name,
        in: "query",
...
function buildResponses(
  ...
  if (route.outputSchema) {
    responses["200"] = {
      ...
      content: {
        "application/json": {
          schema: adapter.toJsonSchema(route.outputSchema),
        },
      },
    };
  } else {
...
- `openapi/mod.ts` — `generateOpenApi()`, `serveOpenApi()`, `valibotAdapter`,
  all builders
- `openapi/openapi.test.ts` — spec shape, operationId generation, parameter
  extraction, error types

## Dependencies

- Valibot-to-JSON-Schema: check if `jsr:@valibot/to-json-schema` exists; if not,
  implement a thin converter for the types used in this framework (object,
  string, number, optional, array, union — the common subset).
