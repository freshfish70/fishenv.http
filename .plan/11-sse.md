...
- **Last-Event-ID**: Passed to handler via `sse.lastEventId` (from request
  header).
...
class SseRouteBuilder<Ctx, Params> {
  ...
  ): SseRouteBuilder<Ctx & NewCtx, Params>;
  param<Name extends string, S extends AnySchema>(
    name: Name,
    schema: S,
  ): SseRouteBuilder<Ctx, MergeParam<Params, Name, S>>;
    ...
    try {
      ...
      for (const handler of errorHandlers) {
        ...
        if (res) {
          ...
          console.error(
            "[fishenv.sse] Handler error after stream started:",
            err,
          );
          break;
        }
...
- `sse/mod.ts` — `SseController`, `SseRouteBuilder`, `withSse()`, SSE dispatch
  logic
- `sse/sse.test.ts` — multiline split, Last-Event-ID, keepalive, cleanup on
  disconnect
